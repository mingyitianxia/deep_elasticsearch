...
【medcl】你应该在 mapping 里面使用 stconvert，确保索引的时候就已经转成了简体，查询的时候，也同样使用这个 analyzer 进行统一成简体，不就成了。只是查询的时候用，索引里面还是繁体当然会匹配不上的。

# 18、请问如何基于query 后的score来搜索
